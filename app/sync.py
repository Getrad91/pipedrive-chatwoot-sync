...
def exponential_backoff_retry(func, max_retries: int = RETRY_ATTEMPTS, base_delay: int = RETRY_DELAY):
    ...
    for attempt in range(max_retries):
        try:
            ...
            logging.getLogger(__name__).warning(f"Attempt {attempt + 1} failed, retrying in {delay:.2f}s: {str(e)}")
            time.sleep(delay)
...
def log_performance_metrics(operation: str, start_time: float, api_calls: int = 0, db_operations: int = 0):
    """Log performance metrics for operations"""
    logger = logging.getLogger(__name__)
    duration = time.time() - start_time
    memory_usage = psutil.Process().memory_info().rss / 1024 / 1024  # MB
    
    logger.info(f"📊 Performance - {operation}: {duration:.2f}s, API calls: {api_calls}, DB ops: {db_operations}, Memory: {memory_usage:.1f}MB")
    ...
    while True:
        try:
            ...
            def make_request():
                nonlocal api_calls
                api_calls += 1
                return pipedrive_session.get(f"{PIPEDRIVE_BASE_URL}/organizations", params=params)
        ...
        with conn.cursor() as cursor:
            ...
            for i, org in enumerate(organizations):
                ...
                if len(batch_data) >= BATCH_SIZE or i == len(organizations) - 1:
                    ...
                    db_operations += 1
                    logger.info(f"Inserted batch of {len(batch_data)} organizations")
                    batch_data = []
...
def sync_to_chatwoot():
                ...
                try:
                    ...
                    if existing_contact is None:
                        ...
                        def search_contact():
                            ...
                            return chatwoot_session.get(search_url, params=search_params, headers=search_headers)
                        
                        search_response = exponential_backoff_retry(search_contact)
                    ...
                    if existing_contact:
                        ...
                        def update_contact():
                            ...
                            return chatwoot_session.put(update_url, json=contact_data, headers=update_headers)
                        ...
                        def create_contact():
                            ...
                            return chatwoot_session.post(create_url, json=contact_data, headers=create_headers)
                    ...
                    if response.status_code in [200, 201]:
                        ...
                        if chatwoot_id and customer_database_inbox_id:
                            try:
                                def assign_contact():
                                    ...
                                    return chatwoot_session.post(assign_url, json=assign_data, headers=assign_headers)
                                
                                assign_response = exponential_backoff_retry(assign_contact)
                        ...
                        batch_updates.append((chatwoot_id, org['pipedrive_org_id']))
                        synced_count += 1
    ...
    log_performance_metrics("Chatwoot Sync", start_time, api_calls, db_operations)
